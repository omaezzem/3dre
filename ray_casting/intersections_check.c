...
/*   Updated: 2025/12/07 11:52:32 by omaezzem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	h_iteration_f(t_hcheck *h, t_cub *cub, double *hit_x, double *hit_y)
{
	while (h->iterations++ < 1000)
	{
		...
		if (h->check_y < 0 || h->check_y >= cub->ray.map_y * T_SIZE ||
			h->intersection_x < 0 || h->intersection_x >= cub->ray.map_x * T_SIZE)
			break;
		if (is_wall(cub, h->intersection_x, h->check_y))
		{
			*hit_x = h->intersection_x;
			*hit_y = h->intersection_y;
			return ft_calcul_vitaghors(cub, h->intersection_x, h->intersection_y);
		}
...
double h_check(t_cub *cub, double *hit_x, double *hit_y, double angle)
{
	...
	h.intersection_x = cub->player.pos_x + (h.intersection_y - cub->player.pos_y) / tan(angle);
	h.step_y = T_SIZE;
	if (h.f_up)
		h.step_y = -T_SIZE;
...
double	v_iteration_f(t_vcheck *v, t_cub *cub, double *hit_x, double *hit_y)
{
	while (v->iterations++ < 1000)
	{
		...
		if (v->check_x < 0 || v->check_x >= cub->ray.map_x * T_SIZE ||
			v->intersection_y < 0 || v->intersection_y >= cub->ray.map_y * T_SIZE)
			break;
		if (is_wall(cub, v->check_x, v->intersection_y))
		{
			*hit_x = v->intersection_x;
			*hit_y = v->intersection_y;
			return ft_calcul_vitaghors(cub, v->intersection_x, v->intersection_y);
		}
...
double v_check(t_cub *cub, double *hit_x, double *hit_y, double angle)
{
	...
	v.intersection_y = cub->player.pos_y + (v.intersection_x - cub->player.pos_x) * tan(angle);
	v.step_x = T_SIZE;
	if (v.f_left)
		v.step_x = -T_SIZE;
